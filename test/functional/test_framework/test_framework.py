...
TMPDIR_PREFIX = "butterfly_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave butterflyds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop butterflyds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use butterfly-cli instead of RPC for all commands")
        parser.add_argument("--butterflyd-arg", dest="butterflyd_extra_args", default=[], action="append",
                            help="Pass extra args to all butterflyd instances")
    ...
    def setup(self):
        ...
        fname_bitcoind = os.path.join(
            ...
            "src",
            "butterflyd" + config["environment"]["EXEEXT"]
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "butterfly-cli" + config["environment"]["EXEEXT"]
        )
        self.options.bitcoind = os.getenv("BITCOIND", default=fname_bitcoind)
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=fname_bitcoincli)

        self.extra_args_from_options = self.options.butterflyd_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: butterflyds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'butterflyd', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'butterfly-cli', self.options.bitcoincli) for v in versions]
    ...
    def dynamically_initialize_datadir(self, node_p2p_port, node_rpc_port):
        ...
        with open(os.path.join(new_data_dir, "butterfly.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as butterflyd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in butterfly.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if butterflyd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("butterflyd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if butterfly-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("butterfly-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if butterfly-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("butterfly-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether butterfly-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether butterfly-wallet was compiled."""
...
class ButterflyTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_butterfly_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, evo_count=0):
        ...
        self.set_butterfly_dip8_activation(200)
        ...
        # This is nRequestTimeout in butterfly-q-recovery thread
    ...
    def set_butterfly_dip8_activation(self, activate_after_block):
    ...
    def set_butterfly_llmq_test_params(self, llmq_size, llmq_threshold):
