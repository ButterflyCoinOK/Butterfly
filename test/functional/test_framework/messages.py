...
def butterflyhash(s):
    return butterfly_hash.getPoWHash(s)
...
# Objects that map to butterflyd objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(butterflyhash(r))
            self.hash = butterflyhash(r)[::-1].hex()
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(butterflyhash(r))
            self.hash = int(butterflyhash(r)[::-1].hex(), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in butterflyd indicates these should be deserialized as blocks
