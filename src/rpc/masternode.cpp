...
static void masternode_list_help(const JSONRPCRequest& request)
{
    RPCHelpMan{"masternodelist",
        ...
        "  owneraddress   - Print the masternode owner Butterfly address\n"
        "  payee          - Print the masternode payout Butterfly address (can be additionally filtered,\n"
        "                   partial match)\n"
        "  pubKeyOperator - Print the masternode operator public key\n"
        "  status         - Print masternode status: ENABLED / POSE_BANNED\n"
        "                   (can be additionally filtered, partial match)\n"
        "  votingaddress  - Print the masternode voting Butterfly address\n",
        {
...
static UniValue masternode_payments(const JSONRPCRequest& request, const ChainstateManager& chainman)
{
    ...
    while (vecPayments.size() < uint64_t(std::abs(nCount)) && pindex != nullptr) {
        ...
        CAmount blockSubsidy = GetBlockSubsidy(pindex, Params().GetConsensus(), pindex->nHeight);
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    { "butterfly",               "masternode",             &masternode,             {} },
    { "butterfly",               "masternodelist",         &masternode,             {} },
};
// clang-format on
