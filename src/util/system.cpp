...
const char * const BITCOIN_CONF_FILENAME = "butterfly.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //butterfly-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Butterfly
    // macOS: ~/Library/Application Support/Butterfly
    // Unix-like: ~/.butterfly
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Butterfly";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Butterfly";
#else
    // Unix-like
    return pathRet / ".butterfly";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty butterfly.conf if it does not exist
