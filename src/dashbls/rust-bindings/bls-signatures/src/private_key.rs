...
impl PrivateKey {
    ...
    #[cfg(feature = "butterfly_helpers")]
    pub fn generate_butterfly(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "butterfly_helpers")]
    pub fn generate_butterfly_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_butterfly(rng))
            .collect()
    }
