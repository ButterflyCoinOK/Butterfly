...
*Setup instructions for a Gitian build of Butterfly Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Butterfly
...
- [Building Butterfly Core](#building-butterfly-core)
...
Clone the git repositories for Butterfly Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ButterflyCoinOK/Butterfly
git clone https://github.com/butterflypay/gitian.sigs.git
```
...
in the Butterfly Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Butterfly Core
----------------

To build Butterfly Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Butterfly Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/butterfly/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ButterflyCoinOK/Butterfly
...
For example:
```bash
URL=https://github.com/crowning-/butterfly.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit butterfly=${COMMIT} --url butterfly=${URL} ../butterfly/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit butterfly=${COMMIT} --url butterfly=${URL} ../butterfly/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit butterfly=${COMMIT} --url butterfly=${URL} ../butterfly/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the butterfly git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../butterfly/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/butterfly
SIGPATH=/some/root/path/butterfly-detached-sigs

./bin/gbuild --url butterfly=${BTCPATH},signature=${SIGPATH} ../butterfly/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/butterfly-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/butterfly-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/butterfly-osx-build.assert
```
...
[butterfly/gitian.sigs](https://github.com/butterflypay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
