...
- [Development guidelines](#development-guidelines)
    - [General Butterfly Core](#general-butterfly-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Butterfly Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Butterfly Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ButterflyCoinOK/Butterfly/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_butterfly
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_butterfly --log_level=test_suite
$ valgrind -v --leak-check=full src/butterflyd -printtoconsole
...
# A coverage report will now be accessible at `./test_butterfly.coverage/index.html`.
```
...
To profile a running butterflyd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep butterflyd` -- sleep 60
```
...
Butterfly Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`butterflyd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Net threads:
  ...
  - [ThreadSocketHandler (`d-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 17754.
...
However, in open source software such as Butterfly Core, where everyone uses
...
pay attention to for reviewers of Butterfly Core code.

General Butterfly Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Butterfly Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Butterfly Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `butterfly-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `butterfly-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Butterfly Core, the wallet was always
...
- Use *invalid* Butterfly addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
