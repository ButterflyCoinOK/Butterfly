...
The Butterfly Core daemon can be configured to act as a trusted "border
router", implementing the butterfly wire protocol and relay, making
...
The ZeroMQ feature in Butterfly Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building butterflyd:
...
For instance:
    ...
    $ butterflyd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/butterflyd.tx.raw \
...
These options can also be provided in butterfly.conf.
...
From the perspective of butterflyd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into butterflyd directly. Furthermore, no information is
...
using. Butterflyd appends an up-counting sequence number to each
